...
export async function getRecipes({ category, difficulty, sort = 'newest', pageSize = 12, lastDoc = null } = {}) {
  let constraints = [];
...
export async function getRecipesByAuthor(authorId) {
  const q = query(recipesCol, where('authorId', '==', authorId), orderBy('createdAt', 'desc'));
  const snap = await getDocs(q);
...
export async function rateRecipe(uid, recipeId, value) {
  ...
  if (existing.exists()) {
    ...
    await updateDoc(doc(db, 'ratings', ratingId), { value, updatedAt: serverTimestamp() });
  } else {
    ...
    const newAvg = ((recipe.ratingAvg || 0) * (recipe.ratingCount || 0) + value) / newCount;
...
export async function getComments(recipeId) {
  const q = query(commentsCol, where('recipeId', '==', recipeId), orderBy('createdAt', 'desc'));
  const snap = await getDocs(q);
  return snap.docs.map(d => ({ id: d.id, ...d.data() }));
}
